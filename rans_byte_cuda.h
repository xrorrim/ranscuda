...
static inline CUDA_CALLABLE void RansEncInit(RansState* r)
{
    *r = RANS_BYTE_L;
}

// Renormalize the encoder. Internal function.
static inline  CUDA_CALLABLE RansState RansEncRenorm(RansState x, uint8_t** pptr, uint32_t freq, uint32_t scale_bits)
{   
    //上限是((RANS_BYTE_L >> scale_bits) << 8) * freq，下限是RANS_BYTE_L
    //符号频率的总和是 2^scale_bits
...
static inline  CUDA_CALLABLE void RansEncPut(RansState* r, uint8_t** pptr, uint32_t start, uint32_t freq, uint32_t scale_bits)
{
    // renormalize
...
static inline  CUDA_CALLABLE void RansEncFlush(RansState* r, uint8_t** pptr)
{
    uint32_t x = *r;
    uint8_t* ptr = *pptr;
...
static inline  CUDA_CALLABLE void RansDecInit(RansState* r, uint8_t** pptr)
{
    uint32_t x;
    uint8_t* ptr = *pptr;
...
static inline  CUDA_CALLABLE uint32_t RansDecGet(RansState* r, uint32_t scale_bits)
{
    //符号频率的总和是 2^scale_bits
...
static inline  CUDA_CALLABLE void RansDecAdvance(RansState* r, uint8_t** pptr, uint32_t start, uint32_t freq, uint32_t scale_bits)
{
...
typedef struct {
    uint32_t x_max;     // 重新归一化之前的上限 (exclusive)
    uint32_t rcp_freq;  // 固定点的频率倒数，用于快速计算
    uint32_t bias;      // 偏移量
    uint16_t cmpl_freq; // 频率的补数 (1 << scale_bits) - freq
    uint16_t rcp_shift; // 倒数的移位量
} RansEncSymbol;

// Decoder symbols are straightforward.
typedef struct {
    uint16_t start;     // 符号对应区间的起始位置
    uint16_t freq;      // 符号的频率
} RansDecSymbol;

// Initializes an encoder symbol to start "start" and frequency "freq"
static inline  CUDA_CALLABLE void RansEncSymbolInit(RansEncSymbol* s, uint32_t start, uint32_t freq, uint32_t scale_bits)
{
    ...
    //   q     = mul_hi(x, rcp_freq) >> rcp_shift   快速除法
    //   r     = x - q*freq                         余数
    //   x_new = q*M + bias + r                     (new x)
    // plugging in r into x_new yields:
    //   x_new = bias + x + q*(M - freq)
    //        =: bias + x + q*cmpl_freq             (*)
    // cmpl_freq 是频率的补数 (1 << scale_bits) - freq
    //
    ...
    if (freq < 2) {
        //  freq等于1的时候特判bias
        //  freq应该不会的等于0，因为过不了renormalization
...
static inline  CUDA_CALLABLE void RansDecSymbolInit(RansDecSymbol* s, uint32_t start, uint32_t freq)
{
...
static inline  CUDA_CALLABLE void RansEncPutSymbol(RansState* r, uint8_t** pptr, RansEncSymbol const* sym)
{
...
static inline  CUDA_CALLABLE void RansDecAdvanceSymbol(RansState* r, uint8_t** pptr, RansDecSymbol const* sym, uint32_t scale_bits)
{
...
static inline  CUDA_CALLABLE void RansDecAdvanceStep(RansState* r, uint32_t start, uint32_t freq, uint32_t scale_bits)
{
    uint32_t mask = (1u << scale_bits) - 1;
...
static inline  CUDA_CALLABLE void RansDecAdvanceSymbolStep(RansState* r, RansDecSymbol const* sym, uint32_t scale_bits)
{
    RansDecAdvanceStep(r, sym->start, sym->freq, scale_bits);
}

// Renormalize.
static inline  CUDA_CALLABLE void RansDecRenorm(RansState* r, uint8_t** pptr)
{
    // renormalize
    uint32_t x = *r;
    if (x < RANS_BYTE_L) {
